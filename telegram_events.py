...
def new_message_interaction(message, *, chat_id: int, is_private: bool) -> TelegramInteraction | None:
    ...
    return TelegramInteraction(chat_id, event_id, "content", text)


def requested_call_interaction(update, *, self_user_id: int | None) -> TelegramInteraction | None:
    phone_call = getattr(update, "phone_call", None)
    if not isinstance(phone_call, types.PhoneCallRequested):
        return None
...
def missed_call_interaction(update, *, self_user_id: int | None) -> TelegramInteraction | None:
