...
class AdminTemplateTestCase(unittest.TestCase):
    ...
    def test_whatsapp_state_refresh_and_recovery_are_actionable(self):
        ...
        self.assertIn('id="wa-qr-img" alt="QR WhatsApp" style="display:none;', template)
        self.assertIn("function showWaQrImage()", template)
        self.assertIn('image.onload = () => { image.style.display = "inline-block"; };', template)
